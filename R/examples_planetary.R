...
dat$theta1 <- get_theta(dat$period, 1000)
dat$x1 <- get_x(dat$theta1, 3, -8)
dat$y1 <- get_y(dat$theta1, 3, 0)

dat$theta2 <- get_theta(dat$period, 807)
dat$x2 <- get_x(dat$theta2, 2.5, 8)
dat$y2 <- get_y(dat$theta2, 2.5, 0)
...
p2 <- p1 +
  geom_point(aes(join_x, join_y), colour = "green")

#Make one planetary
dat$theta1_p <- get_theta(dat$period, 281) + dat$theta1
dat$x1_p <- get_x(dat$theta1_p, 0.85, dat$x1)
dat$y1_p <- get_y(dat$theta1_p, 0.85, dat$y1)

p3 <- p2 + geom_point(aes(x1_p, y1_p), colour = "lightblue")
...
p4 <- p3 +
  geom_point(aes(join_x_p1, join_y_p1), colour = "lightgreen")
...
dat$theta2_p <- get_theta(dat$period, 313) + dat$theta2
dat$x2_p <- get_x(dat$theta2_p, 0.9, dat$x2)
dat$y2_p <- get_y(dat$theta2_p, 0.9, dat$y2)

ggplot(dat[1:500,], aes(x2_p, y2_p)) + geom_point()

dat$join_x_p2 <- get_join_x(dat$x1_p, dat$y1_p,
                            dat$x2_p, dat$y2_p, 180)

dat$join_y_p2 <- get_join_y(dat$x1_p, dat$y1_p,
                            dat$x2_p, dat$y2_p, 180)

ggplot(dat, aes(join_x_p2, join_y_p2)) +
  geom_point(size = 0.2, colour = "darkgrey") +
  geom_path() +
