...
dat$theta2 <- get_theta(1001:11000, 50.07)
dat$x2 <- get_x(dat$theta2, 6, 8)
dat$y2 <- get_y(dat$theta2, 6, 0)

dat$drawing_point_x2 <- get_drawing_point(dat$x1, dat$y1, dat$x2, dat$y2, 16, 3, return = "x")
dat$drawing_point_y2 <- get_drawing_point(dat$x1, dat$y1, dat$x2, dat$y2, 16, 3, return = "y")

#rotate base
orbit_origin <- c(0, 40)
dat$theta_base <- get_theta(dat$period, 25500)
...
ggplot(dat[1:5000,], aes(-x_r, y_r)) + geom_path(size = 0.3, aes(colour = period)) +
